...
def draw_map(player):
    ...
    for inx, cell in enumerate(CELLS):
        if inx in [0, 1, 3, 4, 6, 7]:
            if cell == player:
...
while True:
    ...
    print("You're currently in room {}".format(player))  # fill it with player position

    draw_map(player)

    print("You can move {}".format(moves))  # fill in with available move
